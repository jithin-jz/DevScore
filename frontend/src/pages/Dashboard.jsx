            ...
            if (err.response?.status === 409) {
                ...
                try {
                    ...
                    if (
                        ...
                        statusRes.data.status === 'analyzing'
                    ) {
                        setAnalyzing(true);
                        pollingStartRef.current = Date.now();
                                ...
                                <section className="ds-panel py-32 flex flex-col items-center justify-center text-center space-y-6">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <p className="text-ds-muted text-[11px] max-w-xs mx-auto leading-relaxed">
                                            We need to scan your GitHub account to calculate your
                                            first engineering score.
                                        </p>
                                    </div>
                                    ...
                                    <div className="ds-panel p-6 md:p-8 space-y-8 md:space-y-12 relative overflow-hidden">
                                        ...
                                        <div className="space-y-8 md:space-y-10 relative px-2">
                                            ...
                                            {[
                                                ...
                                                <div
                                                    key={i}
                                                    className={`flex items-center gap-6 relative z-10 ${step.status === 'Locked' ? 'opacity-30' : 'opacity-100'}`}
                                                >
                                                    <div
                                                        className={`w-8 h-8 rounded-full flex items-center justify-center border transition-all duration-500 ${step.current ? 'bg-ds-brand border-ds-brand shadow-lg shadow-ds-brand/40' : step.status === 'Verified' ? 'bg-ds-success border-ds-success' : 'bg-ds-accent/5 border-ds-border'}`}
                                                    >
                                                        {step.status === 'Verified' ? (
                                                            <CheckedIcon size={14} color="white" />
                                                        ) : (
                                                            <span
                                                                className={`text-[10px] font-black ${step.current ? 'text-white' : 'text-ds-text'}`}
                                                            >
                                                                {5 - i}
                                                            </span>
                                                        )}
                                                    ...
                                                    <div className="space-y-1">
                                                        <div className="text-[10px] font-black uppercase tracking-wider text-ds-text">
                                                            {step.tier}
                                                        </div>
                                                        <div
                                                            className={`text-[8px] font-bold uppercase tracking-widest ${step.current ? 'text-ds-brand' : 'text-ds-muted'}`}
                                                        >
                                                            {step.status}
                                                        </div>
                                                    </div>
                                ...
                                <div className="lg:col-span-8 space-y-8">
                                    ...
                                    {recommendations.length > 0 ? (
                                        ...
                                        <div className="ds-panel py-32 md:py-40 text-center border-dashed border-ds-border flex flex-col items-center gap-4">
                                            ...
                                            <h3 className="text-sm font-black text-ds-text uppercase opacity-40">
                                ...
                                <section className="ds-panel space-y-6">
                                    <div className="flex flex-col items-center text-center space-y-4 pb-4">
                                        <img
                                            ...
                                            className="w-20 h-20 rounded-2xl border-2 border-ds-border"
                                        />
                                        <div className="space-y-1">
                                            <h3 className="text-2xl font-black uppercase tracking-tight text-ds-text">
                                                {user?.github_username}
                                            </h3>
                                            <p className="text-[10px] text-ds-muted font-bold tracking-widest uppercase">
                                                {user?.email}
                                            </p>
                                        </div>
                                    </div>
                                    <div className="p-4 md:p-5 rounded-lg bg-ds-accent/5 border border-ds-border flex flex-col md:flex-row items-center justify-between gap-4">
                                        <div className="space-y-1">
                                            <p className="text-[11px] font-black uppercase text-ds-text">
                                                Interface Mode
                                            </p>
                                            <p className="text-[10px] font-medium text-ds-muted capitalize">
                                                Current: {theme}
                                            </p>
                                        </div>
                                        <button
                                            onClick={toggleTheme}
                                            className="px-4 py-2 bg-ds-accent text-ds-bg text-[9px] font-black uppercase rounded"
                                        >
                                            Toggle
                                        </button>
                                    </div>
                                    <button
                                        onClick={logout}
                                        className="w-full py-4 bg-ds-bg border border-ds-border hover:border-ds-danger/30 text-ds-muted hover:text-ds-danger text-[10px] font-black uppercase tracking-widest rounded-xl transition-all flex items-center justify-center gap-3"
                                    >
                                ...
                                <section className="ds-panel space-y-8 border-ds-danger/20">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div className="space-y-0.5">
                                            <h3 className="text-sm font-black text-ds-text">
                                                Account Security
                                            </h3>
                                            <span className="ds-label text-ds-danger opacity-70">
                                                Danger Zone
                                            </span>
                                        </div>
                                    </div>
                                    <div className="p-4 md:p-5 rounded-lg bg-ds-danger/[0.03] border border-ds-danger/10 flex flex-col md:flex-row items-center justify-between gap-4">
                                        <div className="space-y-1 text-center md:text-left">
                                            ...
                                            <p className="text-[10px] font-medium text-ds-muted max-w-xs">
                                        ...
                                        {!showConfirmDelete ? (
                                            <button
                                                ...
                                                className="btn-subtle text-ds-danger border-ds-danger/20 hover:bg-ds-danger hover:text-white"
                                            >
                                                Delete
                                            </button>
                                        ) : (
                                            <div className="flex gap-2">
                                                <button
                                                    ...
                                                    className="px-4 py-2 bg-ds-danger text-white text-[9px] font-black uppercase rounded"
                                                >
                                                    Confirm
                                                </button>
                                                <button
                                                    onClick={() => setShowConfirmDelete(false)}
                                                    className="px-4 py-2 bg-ds-accent/5 text-ds-text text-[9px] font-black uppercase rounded border"
                                                >
                                                    Cancel
                                                </button>
                                            </div>
